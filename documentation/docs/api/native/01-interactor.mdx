...
Max buffer size available in buffers pool.

##### buffers_count

Max buffers available in buffers pool.
...
:::note
Interactor callbacks are used for handling callbacks from Dart side on calls from native side.
:::
