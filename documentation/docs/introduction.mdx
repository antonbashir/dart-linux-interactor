...
And so to interact or communicate between dart and linux sides there is a need of the layer between.
...
So I've decided to combine all theese features into the single library for Dart + Linux interaction.
...
Firstly you need to add to your `pubspec.yaml`:
...
It means that if you need to receive string from the native part then you should `allocate` the string in the Dart part and `free` it also in the Dart part.

This limitation came from the principle: requester controls the resources. 

This way provides us to use efficient lock-free single-thread pools and buffers because `allocation` and `free` operations happen on the same thread/isolate.
